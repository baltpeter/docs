...
echo "deb [signed-by=/etc/apt/keyrings/ntfy.gpg] https://archive.ntfy.sh/apt stable main" | tee /etc/apt/sources.list.d/ntfy.list
apt update
apt install ntfy
...
Subscribe to the [releases on GitHub](https://github.com/binwiederhier/ntfy/releases) via RSS.

Check the release notes and upgrade using apt:
